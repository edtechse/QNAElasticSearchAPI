pipeline {
    ...
    environment {
        ...
        IMAGE_REPO_NAME="qna-elasticsearch-repo"
        IMAGE_TAG="latest"
    ...
    stages {   
      stage('Logging into AWS ECR') {
        ...
        stage('Cloning Git') {
            steps {
                checkout([$class: 'GitSCM', branches: [[name: '*/master']], doGenerateSubmoduleConfigurations: false, extensions: [], submoduleCfg: [], userRemoteConfigs: [[credentialsId: '', url: 'https://github.com/edtechse/QNAElasticSearchAPI.git']]])    
            }
        }
